...
if !(_markers isEqualTo []) then 
{
	...
	*/
	...
	{
		Mission_CENTER = getPosATL (_objects select 0);
	} else {
		Mission_CENTER = getPos (_objects select 0);
	};
...
{
	Mission_CENTER = getPosATL (_staticObjects select 0);
};
diag_log format["Mission_CENTER = %1",Mission_CENTER];
...
{
	_missionCoords pushBack Mission_CENTER;
};
